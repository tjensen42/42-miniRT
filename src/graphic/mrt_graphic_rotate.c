...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/09 14:55:44 by tjensen           #+#    #+#             */
/*   Updated: 2022/06/09 14:55:44 by tjensen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_horizontal(t_scene *scene, double rot)
{
	...
	sine = sin(rot * (M_PI / 180.0));
	cosine = cos(rot * (M_PI / 180.0));
...
static bool	cam_dir_up(struct s_cam *cam, t_vec3 img_py, double rot)
{
	...
	if (cam->dir.y > 1.0 - 1e-4)
		return (false);
	else if (cosine <= cos(rot * (M_PI / 180.0)))
		cam->dir = vec3_add(cam->dir, vec3_scale(sin(rot * (M_PI / 180.0)),
					vec3_norm(img_py)));
	else
...
static bool	cam_dir_down(struct s_cam *cam, t_vec3 img_py, double rot)
{
	...
	if (cam->dir.y < -1.0 + 1e-4)
		return (false);
	else if (cosine >= cos((180.0 + rot) * (M_PI / 180.0)))
		cam->dir = vec3_add(cam->dir, vec3_scale(sin(rot * (M_PI / 180.0)),
					vec3_norm(img_py)));
	else
		cam->dir = (t_vec3){0.0, -1.0, 0.0};
